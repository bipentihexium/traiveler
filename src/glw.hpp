	...
	namespace high {
		...
		public:
			...
			inline void getuv(size_t texid, float &u1, float &v1, float &u2, float &v2) const {
				u1 = texid % w / static_cast<float>(w) + 0.0001f;
				u2 = (texid % w + 1) / static_cast<float>(w) - 0.0001f;
				v1 = texid / w / static_cast<float>(h) + 0.0001f;
				v2 = (texid / w + 1) / static_cast<float>(h) - 0.0001f;
			}
